...
- 分享密码：生成分享链接时可设置密码，收信人输入密码后才能查看正文。
- 数据存储加密：`data/letters.json` 会以加密格式保存，避免明文落盘。
- 登录/注册：支持手机号码或邮箱注册；`.env` 中的 `QIAOPI_USERNAME` 是管理员账号。
- 生成限额：普通注册用户每天最多点击百炼生成正文 15 次，按钮会显示今日剩余额度。
- 后台统计：管理员可访问 `/admin` 查看用户百炼点击、生成分享次数和每日总分享量。
- 登录保护：生成器页面和接口可通过账号密码保护，分享页可公开访问或密码保护。
